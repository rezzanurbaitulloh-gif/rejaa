  ...
  return (
    <figure className={`overflow-hidden rounded-2xl border border-white/12 bg-ink ${className}`} data-reveal-none>
      <div className="relative w-full" style={{ aspectRatio: ratio }} role="img" aria-label={label}>
        <Scene kind={kind} />
        <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, transparent 55%, rgba(4,5,8,0.75))" }} />
        {kind === "video" && (
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 400 225" preserveAspectRatio="xMidYMid slice">
        <defs>
          <linearGradient id="sil-sky" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#0b0d18" /><stop offset="55%" stopColor="#3a2b45" />
            <stop offset="78%" stopColor="#c96b3f" /><stop offset="100%" stopColor="#151021" />
          </linearGradient>
        </defs>
        <rect width="400" height="225" fill="url(#sil-sky)" />
        <circle cx="290" cy="150" r="26" fill="#ffd9a8" opacity="0.85" />
        <g fill="#05060b">
          <rect x="0" y="150" width="42" height="75" /><rect x="52" y="128" width="36" height="97" />
          <rect x="98" y="158" width="48" height="67" /><rect x="330" y="135" width="40" height="90" />
        </g>
        <g fill="#ffd9a8" opacity="0.8">
          {Array.from({ length: 24 }).map((_, i) => (
            <rect key={i} x={6 + (i % 8) * 48} y={140 + Math.floor(i / 8) * 22} width="3" height="4" />
          ))}
        </g>
        {/* sosok + ransel */}
        <g fill="#04050a">
          <ellipse cx="150" cy="120" rx="24" ry="26" />
          <path d="M118 225 V160 Q118 128 150 128 Q182 128 182 160 V225 Z" />
          <rect x="176" y="145" width="34" height="52" rx="10" />
          <rect x="150" y="148" width="30" height="10" rx="5" />
        </g>
      </svg>
    );
  }
